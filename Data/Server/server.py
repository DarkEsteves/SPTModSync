...
JSON_FILE = os.path.join(BASE_DIR, 'versions.json')
...
def log(message, level="INFO"):
    ...
    try:
        if _injected_log_event is not None:
            _injected_log_event("server_dl" if level == "INFO" and "Download" in message else "server_dl_err" if level == "ERROR" else "server_start" if "started" in message else "server_stop" if "stopped" in message else "server_start" if "start" in message.lower() else "server_stop", level, message)
    except Exception:
        pass

_injected_log_event = None
...
<body>
    <div class="container">
        <div class="main-border">
            <h1>MOD MANAGER<span class="sub">ADMIN_PANEL v1.1.1 [ CHUNKY ]</span></h1>
            ...
            <div class="panel">
                ...
                <form id="upload-form">
                    <label>SERVER IP (Download Endpoints):</label>
                    ...
                    <div id="drop-zone">[ DRAG A .ZIP FILE OR CLICK ]</div>
                    ...
                    <button type="submit" id="btn-submit" class="btn-primary">DEPLOY PATCH</button>
                </form>
            </div>
            ...
            <div class="panel">
                ...
                <h2>REGISTERED VERSIONS ON THIS SERVER</h2>
...
<script>
        ...
        function setSelectedFile(file) {
            if (!file) return;
            selectedFile = file;
            fileName.innerText = '>>> READY: ' + file.name.toUpperCase();
        }
        ...
        uploadForm.addEventListener('submit', async (e) => {
            ...
            if (!selectedFile) return alert("[ERROR] Please select a ZIP file.");
            ...
            const files_to_delete = filesToDeleteRaw.split(/\\r?\\n/).map(l => l.trim()).filter(l => l);
...
def get_file(filename):
    ...
    try:
        ...
        pass
    resp = send_from_directory(UPLOAD_FOLDER, safe_name)
    elapsed = max(0.01, time.time() - t0)
    speed = size_mb / elapsed
    log(f"Served download: {safe_name} ({size_mb:.1f} MB) em {elapsed:.1f}s ({speed:.2f} MB/s) | IP={request.remote_addr}")
    return resp
